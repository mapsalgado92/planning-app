...
const CapacityViewer = ({ capacity, data }) => {
  return (
    <div style={{ overflowX: "scroll" }} className="d-flex flex-row text-nowrap text-center w-100 ">
      <div className="d-flex flex-column bg-white sticky-horizontal border-end text-end">
        <h6 className="sticky-header-2 bg-dark text-white text-center mb-0 py-1 px-2">Field</h6>
      ...
      </div>
      {
        capacity.output && capacity.output.map(weekly => <div key={weekly.week.code} className="d-flex flex-column">
          <h6 className={"sticky-header-1 text-white border-end py-1 px-2 p-sticky mb-0 " + (data.weeks.find(week => {
            let today = new Date()
